...
const { create } = await import('../src/parts/NodeForkedProcessRpcClient/NodeForkedProcessRpcClient.js')

test('create - creates rpc client with node forked process', async () => {
  const mockIpc = {
    invoke: jest.fn(),
    send: jest.fn(),
  }
