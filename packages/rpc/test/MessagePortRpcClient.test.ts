...
const { create } = await import('../src/parts/MessagePortRpcClient/MessagePortRpcClient.ts')
...
test('create - creates rpc client with electron message port', async () => {
  ...
  expect(mockIpcChild.listen).toHaveBeenCalledWith(expect.anything(), { port: mockMessagePort })
